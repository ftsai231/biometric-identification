...
addpath(genpath('F:\University of Florida\first year 2\Biometric identification\HW\HW5\iriscode\Matching'));


hd_a = zeros(length(mask_p_f), length(mask_g_f));

for i = 1:length(mask_p_f)
    for j = 1:length(mask_g_f)
        hd_a(i,j) = gethammingdistance2(template_p_f{i,1}, mask_p_f{i,1}, template_g_f{j,1}, mask_g_f{j,1},1);
    end
end
