...
addpath(genpath('F:\University of Florida\first year 2\Biometric identification\HW\HW5\iriscode\Segmentation'));
addpath(genpath('F:\University of Florida\first year 2\Biometric identification\HW\HW5\iriscode\Normal_encoding'));
addpath(genpath('F:\University of Florida\first year 2\Biometric identification\HW\HW5\with_filter\filtered_probe'));


for i = 1:100
    matFilename = sprintf('filtered_probe_%d.png',i);
    [template_p_f{i,1}, mask_p_f{i,1}] = createiristemplate(matFilename);
end



function [template_p_f, mask_p_f] = createiristemplate(eyeimage_filename)

% path for writing diagnostic images
global DIAGPATH
DIAGPATH = 'F:\University of Florida\first year 2\Biometric identification\HW\HW5\with_filter\image\probe_filter';
...
[template_p_f mask_p_f] = encode(polar_array, noise_array, nscales, minWaveLength, mult, sigmaOnf); 
end
